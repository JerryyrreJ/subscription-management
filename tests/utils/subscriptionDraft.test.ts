...
test('reads items from {subscriptions:[...]} and from a bare array', () => {
  const item = { name: 'Netflix', category: 'Streaming', amount: 15.99, currency: 'USD', period: 'monthly', nextPaymentDate: '2026-07-01' };
...
test('drops items without a name', () => {
  const result = normalizeDrafts({ subscriptions: [{ amount: 10, currency: 'USD', period: 'monthly' }, { name: 'Spotify', amount: 9.99, currency: 'USD', period: 'monthly', nextPaymentDate: '2026-07-01' }] }, TODAY);
...
test('defaults and flags unknown currency and period', () => {
  const { drafts } = normalizeDrafts([{ name: 'X', amount: 5, currency: 'XYZ', period: 'weekly', nextPaymentDate: '2026-07-01' }], TODAY);
...
test('guesses a missing next renewal date and migrates a legacy last payment date', () => {
  const missing = normalizeDrafts([{ name: 'X', amount: 5, currency: 'USD', period: 'monthly' }], TODAY);
  assert.equal(missing.drafts[0].nextPaymentDate, TODAY);
  assert.ok(missing.drafts[0].warnings.includes('nextPaymentDate_guessed'));

  const legacy = normalizeDrafts([{ name: 'X', amount: 5, currency: 'USD', period: 'monthly', lastPaymentDate: '2026-06-01' }], TODAY);
  assert.equal(legacy.drafts[0].nextPaymentDate, '2026-07-01');
  assert.equal(legacy.drafts[0].warnings.includes('nextPaymentDate_guessed'), false);
});

test('handles amount edge cases', () => {
  const missing = normalizeDrafts([{ name: 'X', currency: 'USD', period: 'monthly', nextPaymentDate: '2026-07-01' }], TODAY);
  assert.equal(missing.drafts[0].amount, 0);
  assert.ok(missing.drafts[0].warnings.includes('amount_missing'));

  const huge = normalizeDrafts([{ name: 'X', amount: MAX_SUBSCRIPTION_AMOUNT + 100, currency: 'USD', period: 'monthly', nextPaymentDate: '2026-07-01' }], TODAY);
...
test('keeps a valid custom interval and flags a missing one', () => {
  const valid = normalizeDrafts([{ name: 'X', amount: 5, currency: 'USD', period: 'custom', customDate: '45', nextPaymentDate: '2026-07-15' }], TODAY);
  assert.equal(valid.drafts[0].customDate, '45');
  assert.ok(!valid.drafts[0].warnings.includes('customDate_missing'));

  const invalid = normalizeDrafts([{ name: 'X', amount: 5, currency: 'USD', period: 'custom', nextPaymentDate: '2026-07-01' }], TODAY);
...
test('caps the number of drafts', () => {
  const many = Array.from({ length: 80 }, (_, i) => ({ name: `S${i}`, amount: 1, currency: 'USD', period: 'monthly', nextPaymentDate: '2026-07-01' }));
