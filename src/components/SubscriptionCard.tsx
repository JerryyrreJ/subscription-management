...
export function SubscriptionCard({ subscription, index, onClick, onAutoRenew }: SubscriptionCardProps) {
 ...
 <div
 onClick={onClick}
 className="min-h-[200px] sm:min-h-[216px] bg-white dark:bg-[#1a1c1e] rounded-3xl shadow-fey p-4 sm:p-6 transform transition-all duration-300 hover:border-emerald-500 cursor-pointer app-theme-card"
 role="button"
 tabIndex={0}
 ...
 <h3 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-white truncate app-theme-text-primary">{subscription.name}</h3>
 <div className="flex items-center gap-2 min-w-0">
 <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 app-theme-text-muted truncate">{getCategoryDisplayName(subscription.category, t)}</span>
 {isTrial && (
 ...
 <div className="text-lg sm:text-2xl font-semibold text-gray-900 dark:text-white tracking-tight tabular-nums app-theme-text-primary">
 {formatCurrency(subscription.amount, subscription.currency || 'CNY')}
 </div>
 <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 app-theme-text-muted">{periodLabel}</span>
 </div>
 </div>

 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700 space-y-2 sm:space-y-0 app-theme-divider">
 <div className="flex items-center space-x-2">
 <Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 dark:text-gray-500 flex-shrink-0 app-theme-text-muted"/>
 <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-300 app-theme-text-secondary">
 {isTrial
 ...
 <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 sm:h-2 overflow-hidden app-theme-progress-track">
 <div
 ...
 <span className="text-xs text-gray-500 dark:text-gray-400 app-theme-text-muted">
 {t(
