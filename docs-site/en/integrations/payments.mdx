...
3. Copy the new Price ID (`price_...`) into the trusted server variable `STRIPE_PRICE_ID`. Optionally keep `VITE_STRIPE_PRICE_ID` only as a legacy server fallback; the browser does not trust it for checkout.
