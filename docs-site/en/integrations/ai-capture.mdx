...
- Supabase migrations through `20260922000100_ai_monthly_quota.sql`, because monthly quota and budget RPCs are required.
...
Monthly quota is per user and is charged only after a successful parse. Monthly budget is workspace-wide and is enforced with an atomic reservation before the provider call, so concurrent requests cannot all enter the model call after observing the same old spend.
...
- Per-user monthly AI capture counters.
...
| `ai_quota_exceeded` | The signed-in user has used this month's quota |
