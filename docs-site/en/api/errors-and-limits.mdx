...
| `400` | `invalid_subscription_category` | Category is not one of the user's existing visible categories |
...
| `allowedValues` | Valid values for fields such as `category`, `currency`, or `period` |
...
```json
{
  "error": {
    ...
    "message": "Field is not writable: id",
    "field": "id",
...
When `invalid_subscription_category` is returned, choose an exact value from `allowedValues`. API and AI tools cannot create or rename categories; if none fits, the user must use the app category manager.
