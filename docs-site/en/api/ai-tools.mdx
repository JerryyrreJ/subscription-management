...
Only assign an existing category exactly as returned by the app. Never create, rename, or invent a category; ask the user to manage categories in the app when none fits.
...
For `invalid_subscription_category`, use an exact category name from `allowedValues`. Do not create a category on the user's behalf.
