...
Never send server-managed fields: id, createdAt, updatedAt. Use nextPaymentDate for the upcoming renewal.
...
| "Are reminders on / remind me 7 days ahead?" | `get_notification_settings` / confirm then `update_notification_settings` |
| "Here is my Bark URL, configure it" | Refuse; point to the [reminders guide](/en/user-guide/reminders) |
...
The public API does not support soft cancel/pause/resume via `status`. If the user asks to cancel a subscription, clarify whether they mean deleting the local tracking record or cancelling the service outside this app.

Bark URL, test push, timeZone, and locale also cannot be configured through the public API; point the user to the web app and the [reminders guide](/en/user-guide/reminders).
...
```text
I found Netflix (33333333-3333-4333-8333-333333333333), currently monthly. I will change period to yearly while keeping the upcoming renewal date. Confirm?
```
