...
| `PATCH /api/v1/subscriptions/{id}` | Change writable fields | Medium | Required for AI agents |
...
Responses include a read-only `status` (`active`, `paused`, or `cancelled`), and list queries may filter by it. The public API does **not** cancel, pause, or resume subscriptions by writing `status`. Use `DELETE` when the user wants the tracked record removed.

Only `active` subscriptions count toward active spend in the [analytics](/en/api/analytics) endpoints.
