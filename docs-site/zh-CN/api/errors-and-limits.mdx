...
| `400` | `invalid_subscription_category` | 分类不是用户已有的可见分类 |
...
| `allowedValues` | `category`、`currency` 或 `period` 等字段的合法值 |
...
收到 `invalid_subscription_category` 时，只能从 `allowedValues` 中选择完全一致的分类名称。API 和 AI 工具不能创建或重命名分类；如果都不合适，需要用户在应用内的分类管理器中处理。
