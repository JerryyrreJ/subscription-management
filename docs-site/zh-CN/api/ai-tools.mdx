...
删除前，必须重复订阅名称和 id。
不要发送服务端管理字段：id、createdAt、updatedAt。下次续费日期使用 nextPaymentDate。
...
| “提醒开了吗 / 提前 7 天提醒我” | `get_notification_settings` / 确认后 `update_notification_settings` |
| “这是我的 Bark URL，帮我配置” | 拒绝；引导到[续费提醒指南](/zh-CN/user-guide/reminders) |
...
公开 API 不支持通过 `status` 做软取消/暂停/恢复。如果用户说“取消订阅”，需要先确认：是删除本地追踪记录，还是要去外部服务真正取消订阅。

Bark URL、test push、时区和 locale 也不能通过公开 API 配置；引导用户到网页端设置与[续费提醒指南](/zh-CN/user-guide/reminders)。
...
```text
我找到了 Netflix（33333333-3333-4333-8333-333333333333），当前为 monthly。我将把 period 改为 yearly，并保留当前的下次续费日期。确认执行吗？
```
