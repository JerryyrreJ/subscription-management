...
function blogMiddleware(rootDir: string): Connect.NextHandleFunction {
  return (req, res, next) => {
...
function attachBlogMiddleware(server: ViteDevServer | PreviewServer, rootDir: string): void {
  server.middlewares.use(blogMiddleware(rootDir));
}
...
export function blogPlugin(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      attachBlogMiddleware(server, rootDir);
    },
